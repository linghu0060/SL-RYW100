...
BOOL CImageCtrl::RegisterWindowClass(HINSTANCE hInstance)
{
    if(!hInstance) hInstance = AfxGetInstanceHandle();

    WNDCLASS windowclass;
    if( !::AfxCtxGetClassInfo(hInstance, IMAGECTRLNAME, &windowclass) )
    {
           ...
           windowclass.lpszClassName = IMAGECTRLNAME;
...
void CImageCtrl::SetImage(CImage *image)
{
    m_Image = image;
...
void CImageCtrl::SetHeight(float fHeight)
{
...
float CImageCtrl::SetPos(float fPos)
{
...
LRESULT CImageCtrl::OnSetBarColor(WPARAM, LPARAM crBar)
{
...
LRESULT CImageCtrl::OnSetBarBkColor(WPARAM, LPARAM crBarBk)
{
    // call a common routine to redraw window
    CommonPaint();
