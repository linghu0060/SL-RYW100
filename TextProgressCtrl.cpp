...
void CTextProgressCtrl::OnPaint()
{
	...
	if (str.GetLength())
	{
		...
		if( (dwStyle & PBS_VERTICAL) && (szText.cx > (ClientRect.Width() + 2 * EDGE_SPACE)) )
		{
		...
		else
		{
			...
			if( dwStyle & PBS_VERTICAL )
			{
				if( RightRect.Height() >= szText.cy ) {
					ptStart.y = RightRect.bottom - szText.cy;
				}
				else if( MiddleRect.Height() >= szText.cy ) {
					ptStart.y = MiddleRect.top;
				}
				else {
